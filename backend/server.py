...
async def listen_bus_route_data(request):
    ...
    logger.debug("accept connection for bus data")
    while True:
        with contextlib.suppress(ConnectionClosed):
            ...
            try:
                ...
                # other solutions(especially with jsonschema) will be too slow
                bus = Bus(**bus_data)
            except TypeError:
...
async def listen_browser(web_socket, bounds):
    ...
    while True:
        ...
        try:
            ...
            continue
        try:
            windows_bounds = validate_data(
                json_data, Schemes.WINDOW_BOUNDS_SCHEMA
            )
